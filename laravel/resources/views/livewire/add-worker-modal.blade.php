        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    {{ __('Add worker') . ' ' . __('to build') . ' "' . $build->name . '"' }}
                </h3>
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-1 gap-4">
                    ...
                    {{-- Workers --}}
                    <x-select wire:model.defer="worker_id" label="{{ __('Select worker') }}"
                        placeholder="{{ __('Select worker') }}" multiselect :async-data="route('search.workers', ['company_id' => auth()->user()->company_id])" option-label="name"
                        option-value="id" />
